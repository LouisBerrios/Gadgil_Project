...
# Save
ggsave(
  filename = "Gadgil_16S_BETADIVERSITY.SPIDER.png",
  plot = last_plot(),
  device = NULL,
  path = NULL,
  scale = 1,
...
# Calculate centroids for better NMDS illustration
rare2.env <- sample_data(rare2)
scrs <- cbind(as.data.frame(scrs), Condition = rare2.env$Condition)
scrs <- cbind(as.data.frame(scrs), Timepoint = rare2.env$Timepoint)
cent <- aggregate(cbind(NMDS2, NMDS3) ~ Condition + Timepoint, data = scrs, FUN = mean)
segs <- merge(scrs, setNames(cent, c('Condition', 'Timepoint', 'oNMDS2','oNMDS3')),
              by = c('Condition','Timepoint'), sort = FALSE)
se_scores <- aggregate(cbind(NMDS2, NMDS3) ~ Condition + Timepoint, data = scrs, FUN=function(x) sd(x) / sqrt(length(x)))
scrs$BETA <- "Bacterial Beta Diversity"
cent$BETA <- "Bacterial Beta Diversity"
ggplot(scrs, aes(x = NMDS2, y = NMDS3, colour = Timepoint, shape = Condition)) + 
  geom_segment(data = TT, mapping = aes(x = oNMDS2 - NMDS2.x,  y = oNMDS3, xend = oNMDS2 + NMDS2.x, yend=oNMDS3)) + 
  geom_segment(data = TT, mapping = aes(x = oNMDS2,  y = oNMDS3 - NMDS3.x, xend = oNMDS2, yend=oNMDS3 + NMDS3.x)) + 
  geom_point(data = cent, size = 2) + 
  coord_fixed() + theme_bw() + 
  scale_color_manual(values = c("#9a031e", "#0f4c5c", "#F8AFA8", "#f48c06")) +
  annotate("text", x = -0.65, y = 1.5, label = "Stress = 0.0838") +
  annotate("text", x = -0.55, y = 1.3, label = "Condition: p = 0.019") +
  annotate("text", x = -0.54, y = 1.1, label = "Timepoint: p = 0.001") + facet_grid(~BETA) +
  theme(strip.text = element_text(size = 12, face = "bold")) + 
  theme(strip.background = element_rect(fill = "#c18c5d")) + scale_shape_manual(values=c(1, 19))

ggplot(scrs, aes(x = NMDS2, y = NMDS3, colour = Timepoint, shape = Condition, alpha = Condition)) + 
  geom_segment(data = TT, mapping = aes(x = oNMDS2 - NMDS2.x,  y = oNMDS3, xend = oNMDS2 + NMDS2.x, yend=oNMDS3)) + 
  geom_segment(data = TT, mapping = aes(x = oNMDS2,  y = oNMDS3 - NMDS3.x, xend = oNMDS2, yend=oNMDS3 + NMDS3.x)) + 
  geom_point(data = cent, size = 5) + geom_point(size=1) +
  theme_bw() + 
  scale_color_manual(values = c("#9a031e", "#0f4c5c", "#F8AFA8", "#f48c06")) +
  annotate("text", x = -0.645, y = 1.5, label = "Stress = 0.0838") +
  annotate("text", x = -0.55, y = 1.3, label = "Condition: p = 0.019") +
  annotate("text", x = -0.54, y = 1.1, label = "Timepoint: p = 0.001") + facet_grid(~BETA) +
  theme(strip.text = element_text(size = 12, face = "bold")) + 
  theme(strip.background = element_rect(fill = "#c18c5d")) +
  scale_alpha_manual(values =c("Trenched" = 0.3, "Untrenched" = 1))
...
ggplot(scrs, aes(x = NMDS2, y = NMDS3, colour = Timepoint, shape = Condition, alpha = Condition)) + 
  ...
  theme_bw() + 
  scale_color_manual(values = c("#9a031e", "#0f4c5c", "#F8AFA8", "#f48c06")) +
  annotate("text", x = -0.13, y = 0.5, label = "Stress = 0.0838") +
  annotate("text", x = -0.111, y = 0.45, label = "Condition: p = 0.019") +
  annotate("text", x = -0.11, y = 0.4, label = "Timepoint: p = 0.001") + facet_grid(~BETA) +
  theme(strip.text = element_text(size = 12, face = "bold")) + 
  theme(strip.background = element_rect(fill = "#c18c5d")) +
  scale_alpha_manual(values =c("Trenched" = 0.3, "Untrenched" = 1))

ggsave(
  filename = "Gadgil_16S_BETADIVERSITY.SPIDER2.png",
  plot = last_plot(),
  device = NULL,
  path = NULL,
  scale = 1,
...
### New plot with four perpendicular lines emanating from centroids

# Add error "spider legs" for each group (Timepoint-Treatment combination)
for(i in 1:nrow(cent)) {
  ...
  # Add four perpendicular lines for SE (spider plot style)
  # Left-right (x-axis)
  arrows(x_cent - x_se, y_cent, x_cent + x_se, y_cent, angle = 90, length = 0.1, col="red")
  
  # Top-bottom (y-axis)
  arrows(x_cent, y_cent - y_se, x_cent, y_cent + y_se, angle = 90, length = 0.1, col="red")
